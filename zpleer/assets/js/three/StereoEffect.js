...
THREE.headfireStereoEffect = function ( renderer, leftCss, rightCss, zeroDistance ) {   // headfire
	...
	this.focalLength = zeroDistance; // Distance to the non-parallax or projection plane
	...
	Object.defineProperties( this, {
		separation: {
			...
			set: function ( value ) {
				console.warn( 'THREE.zdeskStereoEffect: .separation is now .eyeSeparation.' );
		...
		targetDistance: {
			...
			set: function ( value ) {
				console.warn( 'THREE.zdeskStereoEffect: .targetDistance is now .focalLength.' );
	...
	this.render = function ( scene, camera, swap) {
       ...
       if (!swap) {
        ...
        leftCss.render( scene, _cameraL );  // headfire

		renderer.setScissor( _width, 0, _width, _height );
		renderer.setViewport( _width, 0, _width, _height );
		renderer.render( scene, _cameraR );
		rightCss.render( scene, _cameraR ); // headfire
		...
		renderer.setViewport( 0, 0, _width, _height );
		renderer.render( scene, _cameraR );
        leftCss.render( scene, _cameraR );  // headfire

		renderer.setScissor( _width, 0, _width, _height );
		renderer.setViewport( _width, 0, _width, _height );
		renderer.render( scene, _cameraL );
		rightCss.render( scene, _cameraL ); // headfire
